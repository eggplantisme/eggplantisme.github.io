...
- $$ H(X) \geq 0$$
- $$ H_b(X) = (log_ba)H_a(X)$$ 
这个性质说明熵可以从一个基换到的另一个基，本文默认为以2为底。
...
- $I(X,X)=H(X)-H(X|X)=H(X)$ 据此，熵也叫分布的自信息。
...
- Information inequality
  ...
  ***Proof:***
  ...
  $$\begin{aligned} \because -D(p||q)&=\sum_{x \in \mathcal{X}}p(x)log\ \frac{q(x)}{p(x)}\\&\leq log\sum_{x \in \mathcal{X}}p(x)\frac{q(x)}{p(x)}, by\ Jensen's Inequality\\&=0 \end{aligned}\\ \therefore D(p||q)\geq 0,D(p||q)=0\ iff\  p=q$$
...
- $$\begin{aligned}H(X)\leq log|\mathcal{X}|\end{aligned}\tag{3.1}$$
  ...
  ***Proof:***
  ...
  $$\because D(p||u)=\sum p(x)log\ \frac{p(x)}{u(x)}=log|\mathcal{X}|-H(X)\geq 0 \\ \therefore H(X)\leq log|\mathcal{X}|,H(X)=log|\mathcal{X}|\ iff\ X是\mathcal{X}上的均匀分布$$
